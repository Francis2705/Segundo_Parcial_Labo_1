...
class NivelUno():
    def __init__(self, screen, ancho_alto):
        ...
        self.background = pygame.image.load('imagenes\\fondo1.png')
    ...
    def update_nivel(self, screen):
        if self.player.game_over == 0:
            if len(self.lista_enemigos) == 0: #mato a todos los enemigos 
                ...
                if self.flag_pausa == True: #aca hago las settings
                    ...
                    self.player.update(screen, self.lista_plataformas, self.lista_trampas, self.lista_enemigos, self.lista_monedas, 
                                    self.lista_plataformas_movimiento, self.bullet_group,
                                    self.lista_vidas, self.lista_enemigos_flotantes, self.rectangulo_final, self.flag_sonidos_colision)
                    self.platform.update(screen, self.lista_plataformas)
                    self.enemy.update(screen, self.lista_enemigos, self.lista_plataformas)
    ...
    def reset_level(self):
        ...
        self.platform5 = Platform(400, 500, 150, 50)
        self.platform6 = Platform(800, 400, 100, 50)
        ...
        self.platform_moving = PlatformMoving(870, 650, 60, 50, 1)
        self.platform_moving2 = PlatformMoving(1000, 450, 60, 50, -1)
        self.platform_moving3 = PlatformMoving(600, 300, 80, 50, 1)
        ...
        self.enemy = Enemy(self.platform.rect.x + 1, self.platform.rect.y - 51)
        self.enemy2 = Enemy(self.platform2.rect.x + 1, self.platform2.rect.y - 51)
        self.enemy3 = Enemy(self.platform5.rect.x + 1, self.platform5.rect.y - 51)
        self.enemy4 = Enemy(self.platform6.rect.x + 1, self.platform6.rect.y - 51)
        self.enemy5 = Enemy(self.platform7.rect.x + 1, self.platform7.rect.y - 51)
        lista_enemigos = []
        lista_enemigos.append(self.enemy)
        lista_enemigos.append(self.enemy2)
        lista_enemigos.append(self.enemy3)
        lista_enemigos.append(self.enemy4)
        lista_enemigos.append(self.enemy5)

        self.tramp1 = Tramp(self.platform3.rect.x + 65, self.platform3.rect.y - 30, 1)
        self.tramp8 = Tramp(850, 770, -1)
        self.tramp10 = Tramp(1000, 770, 1)
        lista_trampas = []
        ...
        self.vida4 = Health(650, 150, 20, 20, 1)
        lista_vidas = []
